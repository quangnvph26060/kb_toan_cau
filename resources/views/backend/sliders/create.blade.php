                ...
                @foreach ($sliders as $item)
                    <div class="col-12 col-sm-6 col-md-3 mt-2" id="dropzone-preview-list-{{ $item->id }}">
                        ...
                        <div class="image-preview-container position-relative">
                            ...
                            {{-- <button type="button"
                                class="btn btn-sm btn-danger delete position-absolute top-0 end-0 m-2"></button> --}}
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            // Delete event for dynamically added items
            ...
            window.deleteImage = function(id) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('admin.configurations.sliders.destroy', ':id') }}".replace(':id', id),
                    success: function(response) {
                        console.log(response);
                        // Optionally, remove the image preview from the DOM after delete
                        $('#dropzone-preview-list-' + id).remove();

                        // Show the success toast after delete
                        toastDelete();
                    },
            ...
            var dropzone = new Dropzone(".dropzone", {
                url: "{{ route('admin.configurations.sliders.uploadImage') }}",
                method: "post",
                acceptedFiles: "image/*",
